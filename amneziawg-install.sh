...
install_awg_packages() {
    ...
    # Устанавливаем русскую локализацию для LuCI-протокола AWG 2.0 и новее
...
configure_amneziawg_interface() {
    ...
    if [ "$AWG_VERSION" = "3.0" ]; then
        read -r -p "Enter HeaderProtectionKey (from [Interface]) [optional, leave blank to skip]:"$'\n' AWG_HEADER_PROTECTION_KEY
        read -r -p "Enter ContentPaddingAddition (number or range) [optional, leave blank to skip]:"$'\n' AWG_CONTENT_PADDING_ADDITION
        read -r -p "Enter RekeyAfterTime (number or range) [optional, leave blank to use default]:"$'\n' AWG_REKEY_AFTER_TIME
        read -r -p "Enter RekeyTimeout (number or range) [optional, leave blank to use default]:"$'\n' AWG_REKEY_TIMEOUT
        read -r -p "Enter RejectAfterTime (number or range) [optional, leave blank to use default]:"$'\n' AWG_REJECT_AFTER_TIME
        read -r -p "Enter KeepaliveTimeout (number or range) [optional, leave blank to use default]:"$'\n' AWG_KEEPALIVE_TIMEOUT
        read -r -p "Enter MaxHandshakeAttempts (number or range) [optional, leave blank to use default]:"$'\n' AWG_MAX_HANDSHAKE_ATTEMPTS
        read -r -p "Enter PersistentKeepalive (number or range) [25]:"$'\n' AWG_PERSISTENT_KEEPALIVE
    ...
    # Устанавливаем параметры AWG 2.0 и новее (только если они заданы)
    if [ "$AWG_VERSION" != "1.0" ]; then
